...
const (
	...
	buildTarget = "powerpc64le-linux-gnu"
)
...
var (
	...
	// General purpose registers for parameter passing in ELFv2 ABI
	registers = []string{"R3", "R4", "R5", "R6", "R7", "R8", "R9", "R10"}
	// Floating point registers for parameter passing in ELFv2 ABI
...
func (line *Line) String() string {
	...
	builder.WriteString("\t")
	// For ppc64le, emit WORD directive with binary encoding
...
func (t *TranslateUnit) generateGoAssembly(path string, functions []Function) error {
	...
	for _, function := range functions {
		...
		if len(stack) > 0 {
			...
			builder.WriteString(fmt.Sprintf("\tADD $-%d, R1, R1\n", frameSize))
			stackoffset := 0
			for i := 0; i < len(stack); i++ {
		...
		for _, line := range function.Lines {
			...
			if line.Assembly == "blr" || line.Assembly == "ret" {
				if frameSize > 0 {
					builder.WriteString(fmt.Sprintf("\tADD $%d, R1, R1\n", frameSize))
				}
				if function.Type != "void" {
