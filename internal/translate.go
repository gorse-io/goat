...
func (t *TranslateUnit) compile(args ...string) error {
	...
	args = append(args, t.Target.ClangOptions...)
	compileArgs := append([]string{}, args...)
	compileArgs = append(compileArgs, t.compilerFlags()...)
	compilerPath := GetCompilerPath(t.Target)

	var sourceArgs []string
	switch compilerName {
	case compilerGCC:
		sourceArgs = append([]string{"-S", "-c", t.Source, "-o", t.Assembly}, compileArgs...)
	default:
		sourceArgs = append([]string{"-S", "-target", t.Target.ClangTriple, "-c", t.Source, "-o", t.Assembly}, compileArgs...)
	}

	_, err := RunCommand(compilerPath, sourceArgs...)
	if err != nil {
		return err
	}

	var objectArgs []string
	switch compilerName {
	case compilerGCC:
		objectArgs = append([]string{"-c", t.Assembly, "-o", t.Object}, compileArgs...)
	default:
		objectArgs = append([]string{"-target", t.Target.ClangTriple, "-c", t.Assembly, "-o", t.Object}, compileArgs...)
	}

	_, err = RunCommand(compilerPath, objectArgs...)
	return err
}
...
func (t *TranslateUnit) Header() string {
	...
	builder.WriteString(fmt.Sprintf("// \t%s   %s\n", t.compilerName(), FetchVersion(GetCompilerPath(t.Target))))
...
func FetchVersion(command string) string {
	...
	locs := regexp.MustCompile(`\d+(?:\.\d+)+`).FindAllStringIndex(version, -1)
	var loc []int
	if len(locs) > 0 {
		loc = locs[len(locs)-1]
	} else {
...
func (t *TranslateUnit) compilerFlags() []string {
	switch t.compilerName() {
	case compilerGCC:
		return []string{
			"-finline-limit=1000",
...
// GetGCCPath returns the path to the gcc executable used for code generation.
// If CC is set, it uses that path. Otherwise, native builds use "gcc" and
// cross-builds use the canonical target-prefixed gcc.
func GetGCCPath(target Target) string {
	path := os.Getenv("CC")
	if path != "" {
		return path
	}
